...
var log_file = fs.createWriteStream(log_folder + 'compiler.log', {flags : 'w'});
...
io.sockets.on('connection', function (socket){
	...
	socket.on('compileScripts', function(d){
		...
		var task = function(project, socket){
			...
			if(config != null && config.imagesmoothing != null) dataMainFile += "Game.getContext().imageSmoothingEnabled=" + config.imagesmoothing + ";";
			if(config != null && config.displaymode != null && config.displaymode == "center"){
	...
	socket.on('compileSprites', function(d){
		...
		var task = function(sprite, project, socket, callback){
			fs.readFile(path+"scripts/app.js", "utf-8", function(err, res){
				...
				if(sprite.ressource == null){
					callback();return false;
				}
				
				spritesVars[sprite.name] = {
					ressource: sprite.ressource,
			...
			fs.exists(path + "scripts/app.js", function(ex){
				...
				fs.readFile(path+"scripts/app.js", "utf-8", function(err, newText){
					...
					for(var i = 0; i < data.objects.length; i++) {
						...
						if(objectsCompiled.indexOf(name) > -1) continue;
						objectsCompiled.push(name);
						...
						for(var j = 0; j < propertiesArray.length; j++){
							...
							if(!isNaN(spl[1])) spl[1] = parseFloat(spl[1]);
						...
						switch(type){
							case "sprite":
								...
								if(info == null || info.ressource == null) continue;

								newText += 'var ' + varName + ' = new GameObject([' + size.w + ',' + size.h + ']);';
								newText += varName + '.setRenderer(new SpriteRenderer({name: "' + info.ressource.name + '"}));';
								newText += varName + '.setPosition(' + position.x + ',' + position.y + ');';

								if(properties.layer !== undefined && properties.layer != "0") newText += varName + '.setLayer(' + properties.layer + ');';
								if(properties.opacity !== undefined && properties.opacity != "1") newText += varName + '.setOpacity(' + properties.opacity + ');';
								if(properties.scale !== undefined && properties.scale != "1") newText += varName + '.setScale(' + (properties.scale / 100) + ');';
							...
							case "background":
								var size     = JSON.parse(properties.size);
								var position = JSON.parse(properties.position);
								var color    = (properties.color !== undefined) ? (isNaN(properties.color) ? LZ.decompressFromBase64(properties.color) : properties.color) : "FFF";

								newText += 'var ' + varName + '=new Background({color: "#' + color + '"});';
								newText += varName + '.setPosition(' + position.x + ',' + position.y + ');';
							...
							case "geometricobject":
								var size     = (properties.size) ? JSON.parse(properties.size) : {w:50,h:50};
								var position = JSON.parse(properties.position);
								var shape    = (properties.shape) ? LZ.decompressFromBase64(properties.shape) : "rectangle";
								var color    = (properties.color !== undefined) ? (isNaN(properties.color) ? LZ.decompressFromBase64(properties.color) : properties.color) : "FFF";

								newText += 'var ' + varName + '=new GameObject([' + size.w + ',' + size.h + ']);';
								newText += varName + '.setPosition(' + position.x + ',' + position.y + ');';
								newText += varName + '.setRenderer(new GeometricRenderer({color:"#' + color + '"}));';

								if(properties.layer !== undefined && properties.layer != "0") newText += varName + '.setLayer(' + properties.layer + ');';
								if(properties.opacity !== undefined && properties.opacity != "1") newText += varName + '.setOpacity(' + properties.opacity + ');';
								if(properties.angle !== undefined && properties.angle != "0") newText += varName + '.getRenderer().angle=' + properties.angle + ';';

								newText += varName + '.getRenderer().type="' + shape + '";';
								newText += 'scene' + sceneName + '.registerGameObject("' + name + '", ' + varName + ');';
							break;
							case "tilemap":
								var position    = JSON.parse(properties.position);
						...
						if(properties.behaviors){
								...
								if(obj && typeof obj === "object"){
									for(var i = 0; i < Object.keys(obj).length; i++){
										...
										if(typeof val === "string") 
											obj[key] = LZ.decompressFromBase64(val);
							...
							for(var j = 0; j < Object.keys(behaviors).length; j++){
								...
								params = objectFromBase64(params);
...
utils.log(" ");
utils.log("New Compiler v" + version + " server started !");
